...
public class EmptyTagPolicyTest extends TestCase {
	...
	private String assembleFile(String allowedEmptyTagsSection) {
		return HEADER + DIRECTIVES + COMMON_REGEXPS + COMMON_ATTRIBUTES
				+ GLOBAL_TAG_ATTRIBUTES + TAG_RULES + CSS_RULES
	...
	public void testGetAllowedEmptyTags() throws PolicyException {
		...
		policy = Policy.getInstance(new ByteArrayInputStream(policyFile
				.getBytes()));
	...
	public void testGetAllowedEmptyTags_NoSection() throws PolicyException {
		...
		assertEquals(Constants.defaultAllowedEmptyTags.length, policy.getAllowedEmptyTags().length);
